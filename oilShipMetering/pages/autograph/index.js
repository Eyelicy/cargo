...
Page({
  ...
  subCanvas() {
    ...
    wx.canvasGetImageData({
      ...
      success(res) {
        // 1. png编码
        let pngData = upng.encode([res.data.buffer], that.data.canvasWidth, that.data.canvasHeight)
        // 2. base64编码
        let base64 = wx.arrayBufferToBase64(pngData)
        app.wxRequest('POST', url.electronic_visa, {
          imei: imei,
          uid: uid,
          ...
          if (res.code == 1) {
            wx.navigateTo({
              url: '../evaluate/index?resultid=' + that.data.resultid
            })
          }
        }, (err) => {
